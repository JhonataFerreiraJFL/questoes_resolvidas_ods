...
class ScapegoatTree(BinarySearchTree):
    ...
    def add(self,x):
        ...
        if d > log32(self.q):
            ...
            w = u.parent
            filho = self._size(w)
            pai = self._size(w.parent)
            while 3*filho <= 2*pai:
                if (w.parent.left == w) and (w.parent.right == self.nil):
                    print('left pai')
                    print(w.x)
                    pai +=1
                if w.right == self.nil:
                    ...
                    print(w.x)
                    filho +=1
                if (w.parent.right == w) and (w.parent.left == self.nil):
                    print('right pai')
                    print(w.x)
                    pai +=1
                if w.left == self.nil:
                    ...
                    print(w.x)
                    filho +=1
                if (w.parent.left != self.nil) and (w.parent.right != self.nil):
                    print('size')
                    print(w.x)
                    filho = self._size(w)
                    pai = self._size(w.parent)
