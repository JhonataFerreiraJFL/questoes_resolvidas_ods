...
class ArrayDeque(BaseList):
    ...
    def remove_varios(self, inicio, final):
        if inicio < 0 or inicio >= self.n: raise IndexError()
        if final < 0 or final >= self.n: raise IndexError()
        i = inicio
        k = final
        lenx = final - inicio + 1
        if i < self.n//2:
            while self.a[i-1] is not None or k is inicio:
                self.a[k] = self.a[i-1]
                i -=1
                k -=1
            self.j = lenx
        else:
            while self.a[k+1] is not None or i is final:
                self.a[i] = self.a[k+1]
                i+=1
                k+=1
        self.n -= lenx
